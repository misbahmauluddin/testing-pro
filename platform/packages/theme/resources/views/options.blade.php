...
@section('content')
    ...
    <div class="theme-option-container">
        {!! Form::open(['route' => 'theme.options', 'method' => 'POST']) !!}
            <div class="theme-option-sticky">
                <div class="info_bar">
                    ...
                    <div class="theme-option-action_bar" id="btnsaveadmin-top">
            ...
            <div class="theme-option-sidebar">
                <ul class="nav nav-tabs tab-in-left">
                    @foreach (ThemeOption::constructSections() as $section)
                        <li class="nav-item">
                            <a href="#tab_{{ $section['id'] }}" onclick="hidecostom()" class="nav-link @if ($loop->first) active @endif" data-bs-toggle="tab">@if (!empty($section['icon']))<i class="{{ $section['icon'] }}"></i> @endif {{ $section['title'] }}</a>
                        </li>
                    @endforeach
                    <li class="nav-item">
                        <a href="javascript:void(0)" class="nav-link" onclick="showcostom()"><i class="fas fa-sliders-h"></i> Slider Home Page</a>
                    </li>
                </ul>
            </div>
            <div class="theme-option-main" id="tab-adminruang">
            ...
            <div class="theme-option-sticky" id="btnsaveadmin-bottom">
                <div class="info_bar">
        ...
        <div class="theme-option-main" id="tab-admincustom" style="display: none">
            @include('packages/theme::partials.v-slider')
        </div>
    </div>
@stop
