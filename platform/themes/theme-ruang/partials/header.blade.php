...
<head>
    ...
    <!-- CSS Library-->
    <link rel="stylesheet" href="{{ asset('asset/dist/css/splide.min.css') }}">
                    ...
                    <div class="topbar-right">
                        ...
                        @if (is_plugin_active('real-estate') && RealEstateHelper::isRegisterEnabled())
                            <ul class="topbar-items">
                                ...
                                <li class="login-item"><a href="https://wa.me/+6281280997188" target="_blank" rel="nofollow"><img src="{{ URL::to('uploaded/whatsapp_480px.png'); }}" alt="wa.me" style="margin-bottom: 5px" height="20px"> <span>&nbsp;+62812-8099-7188</span></a></li>

                                @if (auth('account')->check())
                                    @if (auth('account')->user()->type_user === '0' )
                                    ...
                                    @else
                                        <li class="login-item"><a href="{{ url('/customer-dashboard') }}" rel="nofollow"><i class="fas fa-user"></i> <span>{{ auth('account')->user()->name }}</span></a></li>
                                    @endif
                        ...
                        @if (is_plugin_active('real-estate') && RealEstateHelper::isRegisterEnabled())
                            <a class="add-property" style="background: #FFFF; Color: #e12426;" href="{{ route('public.account.properties.index') }}">
...
<header class="topmenu bg-light">
    ...
    <a href="https://wa.me/+6281280997188" class="float-wa" target="_blank">
            ...
            <div class="row">
                <div class="col-12">
                    <nav class="navbar navbar-expand-lg navbar-light">
                        @if (theme_option('logo'))
                            <a class="navbar-brand" href="{{ route('public.single') }}">
                                <img src="{{ RvMedia::getImageUrl(theme_option('logo')) }}"
                                     class="logo" height="65" alt="{{ theme_option('site_name') }}">
                            </a>
                        @endif
                        ...
                        <div class="collapse navbar-collapse justify-content-center animation" id="navbarSupportedContent">
    ...
    @if (is_plugin_active('real-estate') && url()->current() == route('public.single') || ($page && $page->template === 'homepage'))
        ...
        @endphp
        <div class="home_banner" style="background-image: url({{ theme_option('home_banner') ? RvMedia::getImageUrl(theme_option('home_banner')) : Theme::asset()->url('images/banner.jpg') }});height: 71vh"> 
        {{-- height 71vh full page, mobile 112vh --}}

            <section id="image-carousel" class="splide" aria-label="Beautiful Images">
                <div class="splide__arrows">
                    <button class="splide__arrow splide__arrow--prev" style="background: #fff;box-shadow: 0 3px 10px rgb(0 0 0 / 0.2);">
                        <img src="uploaded/sort_left_208px.png" alt="prev" style="margin-bottom: 2px;margin-right: 5px" height="40px">
                    </button>
                    <button class="splide__arrow splide__arrow--next" style="background: #fff;box-shadow: 0 3px 10px rgb(0 0 0 / 0.2);">
                        <img src="uploaded/sort_right_208px.png" alt="next" style="margin-bottom: 2px;margin-left: 5px" height="40px">
                    </button>
                </div>

                <div class="splide__track">
                    <ul class="splide__list">
                        @php $dataslider = DB::table('sliderku')->get(); @endphp
                        @foreach ($dataslider as $dtasldr)
                            <li class="splide__slide mt-4 mb-4">
                                <a href="//{{ $dtasldr->link }}" target="_blank"><img src="{{ URL::to('uploaded/sliderku/' . $dtasldr->img) }}" alt="{{ $dtasldr->name }}" height="320" style="box-shadow: 0 3px 10px rgb(0 0 0 / 0.2);border-radius: 25px;"></a>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            {{-- height 24vh full page, mobile 28vh --}}
            <div class="topsearch" style="margin-top: 24vh;">
                ...
                <form @if (theme_option('enable_search_projects_on_homepage_search', 'yes') == 'yes' && theme_option('default_home_search_type', 'project') == 'project') action="{{ route('public.projects') }}" @else action="{{ route('public.properties') }}" @endif method="GET" id="frmhomesearch">
                    <div class="typesearch pt-0" id="hometypesearch">
                    ...
                    <div class="input-group input-group-lg" style="padding: 25px 25px 16px 25px;background-color: #f5f5f5;border-radius: 25px;">
                        ...
                        <div class="input-group-prepend" style="height:48px;margin-top: 0px;margin-bottom: 10px;">
                            {!! Theme::partial('real-estate.filters.categories-m', compact('categories')) !!}
                        </div>
    
                        <div class="input-group-prepend" style="height:48px;margin-top: 0px">
                            <span class="input-group-text"><i class="far fa-search"></i></span>
                        </div>
                        <div class="keyword-input">
                            <input type="text" class="form-control" name="k" placeholder="{{ __('Masukkan kata kunci pencarian...') }}" id="txtkey" autocomplete="off">
                        ...
                        <div class="location-input" data-url="{{ route('public.ajax.cities') }}">
                            ...
                            <input class="select-city-state form-control" name="location" value="{{ request()->input('location') }}" placeholder="{{ __('Kota, Provinsi') }}" autocomplete="off">
                            <div class="spinner-icon">
                                <i class="fas fa-spin fa-spinner"></i>
                            </div>
                        ...
                        <div class="input-group-append search-button-wrapper" style="margin-bottom: 10px;">
