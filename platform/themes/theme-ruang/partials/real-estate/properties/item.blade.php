        ...
        <p class="threemt bold500">
            @if ($property->rooms_available)
                <span data-toggle="tooltip" data-placement="top" data-original-title="{{ __('Number of rooms') }}">
                    <i><img src="{{ Theme::asset()->url('images/room.svg') }}" alt="icon"></i> <i
                        class="vti">{{ $property->rooms_available }}</i>
                </span>
            @endif
            @if ($property->number_capacity)
                <span data-toggle="tooltip" data-placement="top"
                    data-original-title="{{ __('Capacity') }}"> <i><img
                            src="{{ Theme::asset()->url('images/orang.svg') }}" alt="icon"></i> <i
                        class="vti">{{ $property->number_capacity }}</i></span>
            @endif
            @if ($property->square)
