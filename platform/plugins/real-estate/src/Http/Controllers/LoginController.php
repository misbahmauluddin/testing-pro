    ...
    */
    ...
    public function __construct()
    {
        ...
        if (in_array(session()->get('url.intended'), $pages)) {
            if(session()->get('type_user') == 0){ /// untuk user vendor
                $this->redirectTo = route('public.account.dashboard');
            }elseif(session()->get('type_user') == 1){ /// untuk user costumer
    ...
    public function login(Request $request)
    {
        ...
        if ($this->attemptLogin($request)) {
            $request->session()->put('type_user', auth('account')->user()->type_user);
